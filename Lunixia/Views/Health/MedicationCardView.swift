...
struct MedicationPageView: View {
    ...
    private func medicationRow(_ med: LunixiaMedication) -> some View {
        let dosesToday = med.dosesToday

        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .center, spacing: 12) {
                ZStack {
                    Circle()
                        .stroke(LColors.glassBorder, lineWidth: 4)
                        .frame(width: 44, height: 44)
                    Circle()
                        .trim(from: 0, to: supplyProgress(med))
                        .stroke(LGradients.blue, style: StrokeStyle(lineWidth: 4, lineCap: .round))
                    ...
                    VStack(spacing: 0) {
                        ...
                        Rectangle().fill(LColors.glassBorder).frame(width: 14, height: 0.75)
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text(med.name)
                        ...
                        .foregroundStyle(LColors.textPrimary)
                        .lineLimit(1)
                    MedPillWrap(spacing: 6, rowSpacing: 6) {
                        if let refill = med.refillDate {
                            medPill(text: "REFILL: \(refill.formatted(date: .abbreviated, time: .omitted))", color: LColors.gradientPurple.opacity(0.22))
                        }
                        if med.daysSupply > 0 {
...
private struct MedPillWrap: Layout {
    ...
    func placeSubviews(
        ...
        for subview in subviews {
            let size = subview.sizeThatFits(.unspecified)
            let needsNewRow = currentX > bounds.minX && currentX + spacing + size.width > bounds.maxX
...
struct MedAddEditSheet: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showScheduleSheet) {
            MedScheduleConfigSheet(defaultDoses: $defaultDoses, doseOverrides: $doseOverrides)
        }
    ...
    private func fieldSection<Content: View>(label: String, @ViewBuilder content: () -> Content) -> some View {
        let isTintedSection = label == "details" || label == "configuration"
    ...
    private func save() {
        ...
        case .add:
            onSave(LunixiaMedication(
                name: trimmedName, currentAmount: current, supplyAmount: supply,
        ...
        case .edit(let med):
            med.name = trimmedName; med.currentAmount = current
            med.supplyAmount = supply; med.daysSupply = days
            med.refillDate = includeRefillDate ? refillDate : nil
            med.timesPerDay = defaultDoses; med.doseScheduleOverrides = doseOverrides
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .fill(LColors.glassSurface)
                        .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).strokeBorder(LColors.glassBorder, lineWidth: 0.75))
                )
...
struct MedHistorySheet: View {
    ...
    @ViewBuilder private func historyRow(_ entry: LunixiaMedHistoryEntry) -> some View {
        ...
        .padding(12)
        .background(RoundedRectangle(cornerRadius: 12, style: .continuous).fill(LColors.glassSurface)
            .overlay(RoundedRectangle(cornerRadius: 12, style: .continuous).strokeBorder(LColors.glassBorder, lineWidth: 0.75)))
